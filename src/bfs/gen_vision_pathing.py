        ...
        if (rc.onTheMap(l{encode(x,y)})) {{ // check ({x}, {y})"""
            ...
            if (rc.canSenseLocation(l{encode(x,y)}) && rc.sensePassability(l{encode(x,y)})) {{ """
                    ...
                    dxdy = [
                        ...
                        for dx in range(-1, 2)
                        for dy in range(-1, 2)
                        if (dx, dy) != (0, 0) and dist(x + dx, y + dy) <= radius
                    ]
...
def gen_selection(radius, smaller_radius):
    ...
    for x in range(-7, 8):
        for y in range(-7, 8):
            if (
                smaller_radius < dist(x, y) <= radius
            ):  # on the edge of the radius radius
                out += f"""
