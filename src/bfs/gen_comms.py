        ...
        for attribute in SCHEMA[datatype]["bits"]:
            ...
            for idx in range(SCHEMA[datatype]["slots"]):
                ...
                if start_int == end_int:
                    ...
                    part2_bitstring = "1" * (attribute_bits + rem - 16) + "0" * (
                        32 - attribute_bits - rem
                    )
            ...
            for idx in range(SCHEMA[datatype]["slots"]):
                ...
                if start_int == end_int:
                    bitstring = (
                        "1" * rem
                        + "0" * attribute_bits
                        + "1" * (16 - attribute_bits - rem)
                    )
                    write.append(
                        f"buf{start_int} = (buf{start_int} & {int(bitstring, 2)}) | (value << {(16 - attribute_bits - rem)})"
                    )
                    write.append(f"dirty{start_int} = 1")
                else:
                    part1_bitstring = "1" * rem + "0" * (16 - rem)
                    part2_bitstring = "0" * (attribute_bits + rem - 16) + "1" * (
                        32 - attribute_bits - rem
                    )
                    value1_bitstring = "1" * (16 - rem) + "0" * (
                        attribute_bits + rem - 16
                    )
                    value2_bitstring = "1" * (attribute_bits + rem - 16)
                    write.append(
                        f"buf{start_int} = (buf{start_int} & {int(part1_bitstring, 2)}) | ((value & {int(value1_bitstring, 2)}) >>> {(attribute_bits + rem - 16)})"
                    )
                    write.append(f"dirty{start_int} = 1")
                    write.append(
                        f"buf{end_int} = (buf{end_int} & {int(part2_bitstring, 2)}) | ((value & {int(value2_bitstring, 2)}) << {(32 - attribute_bits - rem)})"
                    )
...
if __name__ == "__main__":
    ...
    # package_name = len(sys.argv) > 1 and sys.argv[1] or 'bfs'
    package_name = "bfs"
